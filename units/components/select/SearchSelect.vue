  ...
  <div class="wrap">
    ...
    <div class="wrap-input" @click="clickOpenDropdown">
      <div v-if="multiple" class="badge-group">
        <template v-for="(item, index) in itemSelected">
...
export default {
  ...
  computed: {
    ...
    hidePlaceholder() {
      return this.itemSelected && this.itemSelected.length > 0 ? true : false
    },
  },
  watch: {
    value(val, oldVal) {
      ...
      handler(val, oldVal) {
        if (!isEqual(val, oldVal)) {
          ...
          if (this.itemSelected && this.itemSelected.length) {
            this.newOptions = differenceBy(this.newOptions, this.itemSelected, 'id')
          }
        }
      },
      deep: true,
  ...
  created() {
    if (this.dataSelected) {
      this.itemSelected = this.dataSelected
    }
  },
  methods: {
    select(option) {
      ...
      const itemSelectedIndex = this.itemSelected ? this.itemSelected.findIndex((el) => el.id === option.id) : -1
      ...
      if (itemSelectedIndex < 0) {
        ...
        this.deleteSelected(option, itemSelectedIndex)
      }
    ...
    handleInput(e) {
      ...
      if (!this.canSearch && this.options && this.options.length) {
